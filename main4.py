...
class CrusherMonitoringApp(QMainWindow):
    ...
    def initUI(self):
        # 设置窗口基本属性
        self.setWindowTitle('Система мониторинга состояния дробилки')
        ...
        # 分析按钮
        self.analyze_btn = QPushButton('Анализ состояния дробилки')
    ...
    def upload_image(self):
        ...
        if file_path:
            ...
            if self.original_image is None:
                QMessageBox.critical(self, "Ошибка", "Не удалось загрузить изображение, проверьте, действителен ли файл")
                return
    ...
    def analyze_image(self):
        ...
        if self.image_path:
            ...
            # 显示保存成功的消息
            self.status_bar.showMessage(f'Изображение с красным контуром сохранено: {self.contour_image_path}')
        ...
        if self.contour_image_path and os.path.exists(self.contour_image_path):
            ...
            if len(sorted_contours) >= 2:
                ...
                # 准备结果文本
                result_text = f"""
                Максимальная длина периметра контура: {largest_perimeter:.2f}
                Вторая по величине длина периметра контура: {second_largest_perimeter:.2f}
                Разница в длине периметра контура: {perimeter_difference:.2f}
                Количество обнаруженных синих контуров: {len(filtered_blue_contours)}
                Оценка состояния дробилки: {status}
                """

                # 更新状态栏
                self.status_bar.showMessage(f'Состояние дробилки: {status}, Количество синих контуров: {len(filtered_blue_contours)}')
            else:
                result_text = "Предупреждение: Не обнаружено достаточно контуров для анализа"
                self.status_bar.showMessage('Анализ не удался: Не обнаружено достаточно контуров')
            ...
            if self.image_path:
                ...
                self.status_bar.showMessage(f'Сохранено окончательное обработанное изображение: {final_image_path}')

            # 在右侧显示处理后的图像
            self.display_image(self.processed_image, self.processed_image_display)
        else:
            QMessageBox.warning(self, "Предупреждение", "Не удалось сохранить или прочитать изображение контура")
