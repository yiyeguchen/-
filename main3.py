...
if len(sorted_contours) >= 2:
    ...
    # 在图像上绘制最大轮廓（蓝色）
    cv2.drawContours(img, [largest_contour], -1, (255, 0, 0), 5)
    # 在图像上绘制次大轮廓（蓝色）
    cv2.drawContours(img, [second_largest_contour], -1, (255, 0, 0), 5)
...
if len(sorted_contours) >= 2:
    ...
    # 保存带轮廓的图像
    cv2.imwrite('rock3.png', img)

    # 计算轮廓周长
    largest_item = sorted_contours[0]
    L1 = cv2.arcLength(sorted_contours[0], True)
    largest_item2 = sorted_contours[1]
    L2 = cv2.arcLength(sorted_contours[1], True)
    print(L1, L2)  # 打印周长值

    # 根据周长差异判断设备状态
    if L1 - L2 > 60:
        print('Необходим ремонт')  # 需要维修
        print('需要维修')
    else:
        print('Установка в норме')  # 安装正常
        print('安装正常')
